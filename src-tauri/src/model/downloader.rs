...
pub async fn start_download(
    ...
    let output_path_buf = Path::new(&file_path).join(format!("{}.bin", file_name));
    ...
    tauri::async_runtime::spawn({
        ...
        async move {
            ...
            if final_payload.download_state == DownloadState::Completed {
                ...
                let model_integrity = compute_file_integrity(output_path.as_str()).await.unwrap();

                let model_integrity_bucket = model_integrity_bucket_state.lock();

                match model_integrity_bucket.set(&output_path, &Json(model_integrity.clone())) {
